...
class SectorContentRenderer {
  ...
  static String distillShortKeyword(String text) {
    ...
    if (lower.contains('yoga & sadhana') ||
        lower.contains('yoga and sadhana')) {
      return 'Yoga & Sadhana';
    }
  ...
  static void _drawRiverPebbles({
    ...
    // Helper to calculate whether 2 whole pebbles fit safely in a bay with natural air gap
    bool canFitTwo(int i1, int i2, double bayWidth) {
      if (bayWidth <= 3.0) return false;
      final fullW1 = pebbleDimensions[i1].halfDeg * 2.0;
      final fullW2 = pebbleDimensions[i2].halfDeg * 2.0;
      final minAirGapDeg = is24HourMode ? 2.0 : 3.0;
      final requiredWidth = fullW1 + minAirGapDeg + fullW2 + 1.2;
      return bayWidth >= requiredWidth;
    }

    // Helper to check if a single whole pebble can fit comfortably with natural margins
    bool canFitSingle(int idx, double bayWidth) {
      if (bayWidth <= 1.5) return false;
      final fullDeg = pebbleDimensions[idx].halfDeg * 2.0;
      return bayWidth >= (fullDeg + 1.0);
    }

    // Assign pebbles into Left Bay and Right Bay based on verified geometric capacity:
    // A chip is drawn only if the whole chip fits. Three chips are no longer forced onto Yoga / Study.
    // If two will not fit in a bay, keep one!
    final leftPebbleIndices = <int>[];
    final rightPebbleIndices = <int>[];

    int leftCapacity = 0;
    if (pebbles.length >= 2 && canFitTwo(0, 1, leftBayWidth)) {
      leftCapacity = 2;
    } else if (pebbles.isNotEmpty && canFitSingle(0, leftBayWidth)) {
      leftCapacity = 1;
    }

    int rightCapacity = 0;
    final rIdx1 = pebbles.length >= 2 ? 1 : 0;
    final rIdx2 = pebbles.length >= 3 ? 2 : (pebbles.length >= 2 ? 1 : 0);
    if (pebbles.length >= 2 && canFitTwo(rIdx1, rIdx2, rightBayWidth)) {
      rightCapacity = 2;
    } else if (pebbles.isNotEmpty && canFitSingle(rIdx1, rightBayWidth)) {
      rightCapacity = 1;
    }

    var nextPebbleIdx = 0;
    if (leftBayWidth >= rightBayWidth) {
      if (leftCapacity >= 1 && nextPebbleIdx < pebbles.length) {
        leftPebbleIndices.add(nextPebbleIdx++);
      }
      if (rightCapacity >= 1 && nextPebbleIdx < pebbles.length) {
        rightPebbleIndices.add(nextPebbleIdx++);
      }
      if (leftCapacity >= 2 && nextPebbleIdx < pebbles.length) {
        leftPebbleIndices.add(nextPebbleIdx++);
      }
      if (rightCapacity >= 2 && nextPebbleIdx < pebbles.length) {
        rightPebbleIndices.add(nextPebbleIdx++);
      }
    } else {
      if (rightCapacity >= 1 && nextPebbleIdx < pebbles.length) {
    ...
    void renderBayPebbles({
      ...
      if (indices.length == 1) {
        ...
        if (bayWidth < minRequiredWidth) {
          // If two will not fit, keep one!
          final idx = idx1;
          final dim = pebbleDimensions[idx];
          final fullW = dim.halfDeg * 2.0;
          final maxAllowedW = math.max(0.0, bayWidth - 1.2);
          final scaleFactor = (maxAllowedW / fullW).clamp(0.85, 1.0);
          final scaledHalf = dim.halfDeg * scaleFactor;
          final minSafeAngle = bayStart + scaledHalf;
          final maxSafeAngle = bayEnd - scaledHalf;
          final targetAngle = minSafeAngle <= maxSafeAngle
          ...
          final pRad = SectorMath.dialAngleToCanvasRadians(pDeg);
          final pR = midR.clamp(
            rIn + (dim.h * scaleFactor) / 2.0 + 2.0,
            rOut - (dim.h * scaleFactor) / 2.0 - 2.0,
          );
